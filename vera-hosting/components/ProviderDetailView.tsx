...
const ProviderDetailView: React.FC<ProviderDetailViewProps> = ({
    ...
    return (
        <div className="w-full flex flex-col h-full bg-slate-50 dark:bg-[#0a0a0a]/60 border-l border-slate-200 dark:border-white/5 overflow-y-auto animate-in slide-in-from-right-8 duration-300">
            <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-white/10 bg-emerald-50 dark:bg-emerald-900/10 flex flex-col sm:flex-row justify-between items-start gap-4 sm:gap-0 sticky top-0 z-10 backdrop-blur-md bg-opacity-90 dark:bg-opacity-90">
                <div className="w-full sm:w-auto">
                    {/* Mobile Only Back Button */}
                    {onClose && (
                        <button onClick={onClose} className="sm:hidden mb-3 text-emerald-700 dark:text-emerald-400 font-medium text-sm flex items-center gap-1 hover:text-emerald-800 transition-colors">
                            <X size={16} /> Back to Explorer
                        </button>
                    )}
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1 leading-tight">{selectedRecord.name}</h3>
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-xs text-slate-500 dark:text-gray-400 mt-1 sm:mt-0">
                        <span className="font-mono bg-white dark:bg-white/5 px-1.5 py-0.5 rounded text-emerald-700 dark:text-emerald-300 border border-slate-200 dark:border-transparent shrink-0">{selectedRecord.npi}</span>
                        <span className="break-words">{selectedRecord.specialty}</span>
                    </div>
                </div>
                {/* Desktop Close Button */}
                {onClose && (
                    <button onClick={onClose} className="hidden sm:block text-slate-400 dark:text-gray-400 hover:text-slate-700 dark:hover:text-white transition-colors p-1 -mr-1">
                        <X size={20} />
                    </button>
                )}
            </div>
