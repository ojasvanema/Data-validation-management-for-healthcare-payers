  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      ...
      <GlassCard className={`border-slate-200 dark:border-emerald-500/20 bg-white/80 dark:bg-[#050505]/80 flex flex-col p-0 overflow-hidden shadow-sm dark:shadow-none transition-all duration-300 ${isCollapsed ? 'h-auto' : 'flex-1'}`}>
        ...
        {isCollapsed ? (
          <div className="p-4 grid grid-cols-4 sm:flex items-center justify-around gap-y-4 sm:h-24 animate-in fade-in slide-in-from-top-2 duration-300">
            {agents.map((agent) => {
      ...
      {/* Mini Status Grid */}
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 h-auto sm:h-20">
        {agents.map((agent) => {
          const Icon = AGENT_ICONS[agent.icon];
          const isActive = agent.status === 'processing';
          const isCompleted = agent.status === 'completed';
          ...
          return (
            <GlassCard
              key={agent.id}
              className={`p-2 sm:p-0 flex flex-col items-center justify-center gap-1 transition-all duration-300 shadow-sm dark:shadow-none
                        ${isActive
