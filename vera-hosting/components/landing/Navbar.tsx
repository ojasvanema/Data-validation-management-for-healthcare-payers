...
const Navbar: React.FC<NavbarProps> = ({ onRequestAccess }) => {
    ...
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white/80 dark:bg-neutral-950/80 backdrop-blur-md border-b border-slate-200 dark:border-white/5 py-4' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Button variant="secondary" size="sm" className="ml-4" onClick={onRequestAccess}>
            Request Access
          </Button>
        </div>
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white dark:bg-neutral-950 border-b border-slate-200 dark:border-white/10 p-6 flex flex-col gap-4 shadow-2xl animate-in slide-in-from-top-4 duration-200">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="text-lg font-medium text-slate-700 dark:text-gray-300 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors py-2 border-b border-slate-100 dark:border-white/5 last:border-0"
              onClick={() => setIsMobileMenuOpen(false)}
            >
              {link.name}
            </a>
          ))}
          <Button className="w-full mt-4" onClick={() => { setIsMobileMenuOpen(false); onRequestAccess?.(); }}>Request Access</Button>
        </div>
      )}
    </nav>
  );
