  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-8">
      ...
      <div className="lg:col-span-3 grid grid-cols-2 lg:grid-cols-4 gap-4">
        <GlassCard className="p-3 sm:p-4 flex flex-col items-center justify-center text-center border-emerald-200 dark:border-emerald-500/20 bg-emerald-50 dark:bg-emerald-900/10 shadow-sm dark:shadow-none">
          <DollarSign className="text-emerald-600 dark:text-emerald-400 mb-2" size={24} />
          <span className="text-emerald-700/60 dark:text-emerald-200/60 text-[10px] sm:text-xs uppercase tracking-wider">Potential ROI</span>
          <span className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">${data.roi.toLocaleString()}</span>
        </GlassCard>
        <GlassCard className="p-3 sm:p-4 flex flex-col items-center justify-center text-center border-red-200 dark:border-red-500/20 bg-red-50 dark:bg-red-900/10 shadow-sm dark:shadow-none">
          <ShieldAlert className="text-red-500 dark:text-red-400 mb-2" size={24} />
          <span className="text-red-700/60 dark:text-red-200/60 text-[10px] sm:text-xs uppercase tracking-wider">Fraud Risk</span>
          <span className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">{data.fraudRiskScore}/100</span>
        </GlassCard>
        <GlassCard className="p-3 sm:p-4 flex flex-col items-center justify-center text-center border-blue-200 dark:border-blue-500/20 bg-blue-50 dark:bg-blue-900/10 shadow-sm dark:shadow-none">
          <Users className="text-blue-500 dark:text-blue-400 mb-2" size={24} />
          <span className="text-blue-700/60 dark:text-blue-200/60 text-[10px] sm:text-xs uppercase tracking-wider">Providers</span>
          <span className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">{data.providersProcessed}</span>
        </GlassCard>
        <GlassCard className="p-3 sm:p-4 flex flex-col items-center justify-center text-center border-orange-200 dark:border-orange-500/20 bg-orange-50 dark:bg-orange-900/10 shadow-sm dark:shadow-none">
          <AlertOctagon className="text-orange-500 dark:text-orange-400 mb-2" size={24} />
          <span className="text-orange-700/60 dark:text-orange-200/60 text-[10px] sm:text-xs uppercase tracking-wider">Discrepancies</span>
          <span className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">{data.discrepanciesFound}</span>
        </GlassCard>
      </div>
      ...
      <GlassCard className="lg:col-span-3 p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <span className="text-[10px] font-semibold text-purple-500 dark:text-purple-400 uppercase tracking-wider bg-purple-50 dark:bg-purple-500/10 px-2 py-1 rounded-full border border-purple-100 dark:border-purple-500/20 w-fit">
            Degradation Agent
          </span>
        </div>
        {(() => {
      ...
      <GlassCard className="lg:col-span-3 p-6 max-h-[300px] overflow-hidden flex flex-col">
        ...
        <div className="overflow-y-auto pr-2 space-y-2 custom-scrollbar">
          {data.agentLogs.map((log, idx) => (
            <div key={idx} className="flex flex-col sm:flex-row gap-1 sm:gap-4 text-sm border-b border-slate-100 dark:border-white/5 pb-3 last:border-0 hover:bg-slate-50 dark:hover:bg-white/5 p-2 rounded transition-colors">
              <div className="flex items-center gap-2 sm:gap-4">
                <span className="text-slate-400 dark:text-gray-500 font-mono text-xs w-20 sm:w-24 shrink-0 sm:pt-1">{new Date(log.timestamp).toLocaleTimeString()}</span>
                <span className="text-emerald-600 dark:text-emerald-400 font-bold w-full sm:w-32 shrink-0 sm:pt-1">{log.agent}</span>
              </div>
              <span className="text-slate-600 dark:text-gray-300 leading-relaxed max-w-full sm:max-w-[70%] mt-1 sm:mt-0">{log.log}</span>
            </div>
          ))}
        </div>
      </GlassCard>
