...
import { ShieldCheck, ChevronLeft, ChevronRight, PlusCircle, LayoutDashboard, Database, History, LogOut, Menu, X } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
        <>
            ...
            <aside className={`
                fixed lg:relative flex flex-col flex-shrink-0 z-50 h-full transition-all duration-300
                bg-white/95 border-r border-slate-200 backdrop-blur-xl shadow-2xl lg:shadow-none
                dark:bg-[#0a0a0a]/95 dark:border-white/5
            ...
            `}>
                <div className={`p-6 border-b border-inherit flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
                    ...
                    {!isCollapsed && (
                        <button onClick={() => setIsCollapsed(true)} className="hidden lg:block p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 text-slate-400 hover:text-emerald-500 transition-colors">
                            <ChevronLeft size={16} />
                        </button>
                    )}
                    {/* Mobile Close Toggle */}
                    <button onClick={() => setIsMobileMenuOpen(false)} className="lg:hidden p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 text-slate-400 hover:text-emerald-500 transition-colors">
                        <X size={20} />
                    </button>
                </div>
                {isCollapsed && (
                    <div className="hidden lg:flex justify-center py-2 border-b border-inherit">
