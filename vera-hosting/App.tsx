...
import { Activity, LayoutDashboard, History, PlusCircle, ChevronRight, ChevronLeft, Leaf, Database, LogOut, ShieldCheck, PlayCircle, Menu } from 'lucide-react';
...
function AppContent() {
    ...
    // Function to process agent activity step-by-step
    const runAgentActivity = async (finalResult: AnalysisResult, runEfficiently: boolean = false) => {
        ...
        // 3. Fraud Detection (part of validation/risk)
    ...
    const handleFilesSelected = async (files: FileUpload[]) => {
        ...
        try {
            ...
            // Run the visual representation using the real data
            await runAgentActivity(result);
    ...
    const loadHistoricalData = async (runEfficiently: boolean = true) => {
        ...
        // Loading processing state for visual effect
        updateAgent(AgentType.ORCHESTRATOR, 'processing', 'Initiating validation sequence...');

        try {
            // We reuse the historical loader instead of real API
            const { fetchBatchData } = await import('./services/apiService');
            const data = await fetchBatchData();
            await runAgentActivity(data, runEfficiently);
        } catch (error) {
    ...
    const handleCSVUpload = async (file: File, runEfficiently: boolean = true) => {
        ...
        try {
            ...
            // Intercepting CSV upload for static site wrapper
            const { fetchBatchData } = await import('./services/apiService');
            const data = await fetchBatchData();
            await runAgentActivity(data);
        } catch (error) {
    ...
    return (
        <div className="flex h-screen overflow-hidden font-sans transition-colors duration-300 bg-slate-50 dark:bg-[#050505] text-slate-900 dark:text-white">
            ...
            <Sidebar
                ...
                currentView={view}
                setCurrentView={(v) => { setView(v); setIsMobileMenuOpen(false); }}
                history={history}
            ...
            ">
                ...
                ">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="hidden sm:flex items-center text-sm breadcrumbs text-slate-400 dark:text-gray-400">
                        ...
                        {/* Mobile Title */}
                        <div className="sm:hidden font-semibold text-slate-900 dark:text-white">
                            {view === 'home' ? 'Home' : view === 'dashboard' ? 'Dashboard' : view === 'explorer' ? 'Explorer' : 'Settings'}
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto w-full scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
                    <div className="max-w-7xl mx-auto h-full flex flex-col p-4 sm:p-8">
