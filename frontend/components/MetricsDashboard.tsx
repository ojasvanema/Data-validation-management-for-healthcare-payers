...
const MetricsDashboard: React.FC<MetricsDashboardProps> = ({ data }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* Predictive Degradation Forecast (New Graph) */}
      <GlassCard className="lg:col-span-3 p-6 min-h-[300px]">
        <h3 className="text-lg font-semibold mb-6 text-white flex items-center gap-2">
          <span className="w-2 h-6 bg-purple-500 rounded-sm"></span>
          Predictive Data Degradation (AI Forecast)
        </h3>
        <p className="text-sm text-gray-400 mb-4">
          Projected data validity score over the next 12 months based on provider specialty volatility.
        </p>
        <ResponsiveContainer width="100%" height={250}>
          <AreaChart data={data.aggregateDecayCurve}>
            <defs>
            ...
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
            <XAxis dataKey="name" stroke="rgba(255,255,255,0.3)" fontSize={12} tickLine={false} axisLine={false} />
            <YAxis domain={[0, 1]} stroke="rgba(255,255,255,0.3)" fontSize={12} tickLine={false} axisLine={false} />
            <Tooltip
              contentStyle={{ backgroundColor: '#022c22', borderColor: '#064e3b', color: '#fff', borderRadius: '8px' }}
            />
            <Area type="monotone" dataKey="value" stroke="#a855f7" strokeWidth={3} fillOpacity={1} fill="url(#colorDecay)" name="Validity Score" />
          </AreaChart>
        </ResponsiveContainer>
      </GlassCard>
